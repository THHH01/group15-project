...
const taoToken = (user) => {
  ...
  return jwt.sign(payload, biMat, { expiresIn: '1h' });
};
...
const dangKy = async (req, res) => {
  try {
    ...
    const daTonTai = await User.findOne({ email: email.toLowerCase() });
    if (daTonTai) {
      return res.status(409).json({ thongBao: 'Email đã tồn tại. Vui lòng sử dụng email khác.' });
    }
...
const dangNhap = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      token,
      nguoiDung: buildTokenPayload(nguoiDung)
    });
  } catch (error) {
